...
getparams <- function(fit, condition) {
  ...
  fitdf$condition <- condition
  fitdf
}


load("model_fits/regression_1m_cond0.RData")
df0 <- getparams(fit, 0)
ypred0 <- extract(fit)$y_sim
...
summary0 <- as.data.frame(monitor(extracted, digits_summary=2, print=FALSE))
load("model_fits/regression_1m_cond1.RData")
df1 <- getparams(fit, 1)
ypred1 <- extract(fit)$y_sim
extracted=extract(fit, permuted=FALSE, pars=c("bias_mean", "bias_sd",
                                              "expectation_mean", "expectation_sd",
                                              "horizon_mean", "horizon_sd",
                                              "trial_mean", "trial_sd",
                                              "interaction_mean", "interaction_sd"))
summary1 <- as.data.frame(monitor(extracted, digits_summary=2, print=FALSE))
load("model_fits/regression_1m_cond2.RData")
df2 <- getparams(fit, 2)
ypred2 <- extract(fit)$y_sim
extracted=extract(fit, permuted=FALSE, pars=c("bias_mean", "bias_sd",
                                              "expectation_mean", "expectation_sd",
                                              "horizon_mean", "horizon_sd",
                                              "trial_mean", "trial_sd",
                                              "interaction_mean", "interaction_sd"))
summary2 <- as.data.frame(monitor(extracted, digits_summary=2, print=FALSE))
load("model_fits/regression_4m_cond0.RData")
df3 <- getparams(fit, 3)
ypred3 <- extract(fit)$y_sim
extracted=extract(fit, permuted=FALSE, pars=c("bias_mean", "bias_sd",
                                              "expectation_mean", "expectation_sd",
                                              "horizon_mean", "horizon_sd",
                                              "trial_mean", "trial_sd",
                                              "interaction_mean", "interaction_sd"))
summary3 <- as.data.frame(monitor(extracted, digits_summary=2, print=FALSE))
load("model_fits/regression_4m_cond1.RData")
df4 <- getparams(fit, 4)
ypred4 <- extract(fit)$y_sim
extracted=extract(fit, permuted=FALSE, pars=c("bias_mean", "bias_sd",
                                              "expectation_mean", "expectation_sd",
                                              "horizon_mean", "horizon_sd",
                                              "trial_mean", "trial_sd",
                                              "interaction_mean", "interaction_sd"))
summary4 <- as.data.frame(monitor(extracted, digits_summary=2, print=FALSE))
load("model_fits/regression_4m_cond2.RData")
df5 <- getparams(fit, 5)
ypred5 <- extract(fit)$y_sim
extracted=extract(fit, permuted=FALSE, pars=c("bias_mean", "bias_sd",
                                              "expectation_mean", "expectation_sd",
                                              "horizon_mean", "horizon_sd",
                                              "trial_mean", "trial_sd",
                                              "interaction_mean", "interaction_sd"))
summary5 <- as.data.frame(monitor(extracted, digits_summary=2, print=FALSE))
