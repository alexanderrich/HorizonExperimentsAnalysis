data {
  ...
  vector[N] patchHorizon; // horizon of patch (length or frequency)
...
transformed parameters {
  ...
  subj_horizon <- subj_horizon_raw * horizon_sd + horizon_mean;
...
generated quantities {
  ...
  }
  {
    ...
    for (n in 1:N) {
      ...
      predictor <- b_bias + b_expectation .* expectation[n] + b_horizon .* patchHorizon[n] +
        b_trial .* patchTrialNum[n] + b_interaction .* patchHorizon[n] .* patchTrialNum[n];
      p <- 1 ./ (1 + exp(-predictor));
